...
/*************************************************************************/
string VisitantePrecio::getInformacion(Cliente* cliente) {
	int descuento = (*cliente).getDescuento();
    precioTotal = precioTotal - (precioTotal*descuento/100.0);
	string precioTotalStr = "Precio del equipo aplicando descuento del " + to_string(descuento) +"%: " + to_string(precioTotal);
